...
\Todo{ggf dann hier auch ne Quelle}
...
As shown by \citewithauthor{wohlrab2019ImprovingConsistency}, inconsistencies \Todo{ggf dann hier auch die ArDoCo Inconsistency Detection (ID) nennen; die nutzt TLR für ID: https://ardoco.de/approaches/inconsistency-detection/} in wording and language are quite common during different stages of development.
...
\Todo{Möglich hier \acl{LiSSA} zu nutzen ohne das Zitat?}
\Todo{ggf wäre hier LiSSA als Abschnitt gut, damit man einschätzen kann, wo man prompts jetzt in der TLR verwendet.}

\Todo{Bild hier nochmal platzieren statt auf alte Grafik zu verweisen?}
...
First, the artifacts will be pre-processed into elements.
This element type is used for all supported domains.
While the performance of later steps might be diminished or improved with different artifact types, the processing remains the same.
Using embedding \Todo{What are embeddings?} \LLMs vector representations are created, which in turn will be used to find similar element candidates.

In the classical \LiSSA evaluation pipeline without prompt optimization, the source elements are combined with their target element candidates.
They will be prompted to an \LLM to determine whether a \TL exists between the source element and the candidate.
This is done using a so-called classification prompt.
The classification results are aggregated and post-processed into a set of \TLs. 

The new optimization step, highlighted in light blue in \autoref{fig:prompt_optimization_pipeline}, is an iterative pipeline step to alter the classification prompt. 
...
\Todo{Bitte keine vorwärts-referenzen.}
...
\Todo{für die abläufe wären kleine grafiken gut}
