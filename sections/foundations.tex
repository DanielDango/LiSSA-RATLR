...
They will be preprocessed with the configured preprocessor into elements.
Importantly they contain the content of the artifact as text, which will be used in later steps.
Next, embeddings for these elements are created.
Embeddings are vector representations of the textual content held by the elements.
\Todo{Source?}
Utilizing the elements and their embeddings, \elementstores are initialized.
They provide functionality to search for similar elements based on their embeddings.
This \retrievalstrategy can be configured.
