...
The three different \APE approaches and \ToT promting proposed in \autoref{ch:Approach} are evaluated in this chapter.
To make the results comparable, the same datasets as in \citewithauthor{hey2025RequirementsTraceability} are used.
In \autoref{approach:sec:evaluation}, the evaluation approach is described.
\autoref{sec:Evaluation:setup} describes the general evaluation setup, including datasets and \LLMs used.
First, \autoref{sec:Evaluation:tot_prompting} presents the results of \ToT prompting for \TLR in the \LiSSAf.
Then, \autoref{sec:Evaluation:naive_optimization}, \autoref{sec:Evaluation:simple_feedback_optimization}, and \autoref{sec:Evaluation:varying-the-optimization-prompt} present the naive prompt optimization approaches.
The findings of gradient-based prompt optimization are presented in \autoref{sec:Evaluation:gradient_based_optimization}.
They are also tested for their generalizability to new datasets in \autoref{sec:Evaluation:cross_dataset_evaluation}.
...
Further the \fone using a fixed $\beta$ in \autoref{eq:f_beta}, are used commonly~\cite{fuchss2025LiSSAGeneric, hey2025RequirementsTraceability}.
\directQuote[sec. 7.1]{
    Precision measures the ratio of correctly predicted links and all predicted links.
    Recall measures the ratio of correctly predicted links and all relevant documents.
    It shows an approach’s ability to provide correct trace links.
    The \fone is the harmonic mean of precision and recall.
    The Metrics are calculated in the following way, where TP is true positives, FP is false positives, and FN is false negatives.
}{ewald2024RetrievalAugmentedLarge}
These three measures are also used in my thesis.
...
This way comparison is very simple.
...
To evaluate the \APE algorithms proposed in \autoref{ch:Approach}, a multitude of different datasets will be used.
They are taken from the replication package of \citewithauthor{hey2025ReplicationPackage}.
Some of the gold standard files are modified to provide consistency between artifact naming and the gold standard reference.
The actual contents are not affected by this.
The \ac{CCHIT} dataset is omitted, as it differs from the others in that it does not link high-level artifacts with low-level artifacts.
...
Several \LLMs are be used.
The focus is on \OAI's \gpt and \gptmini models.
...
In addition to the four \LLMs used by \citewithauthor{hey2025RequirementsTraceability} in their preceding work. \gptf is also briefly addressed, as a newer \LLM.
As of this publication, it is the most up-to-date text embedding model by \OAI, thus possibly improving future comparability.

The \LiSSAf enables the usage of different embedding models for evaluation.
However, previous work~\cite{fuchss2025LiSSAGeneric, hey2025RequirementsTraceability} exclusively uses \ac{text-embedding} by \OAI.
Thus, to ensure comparability of results, this embedding model will also be used in this evaluation.
...
Future work may address the current work of \citewithauthor{fuchss2025RetrievalStudya}.
...
The datasets and exact configurations used for each evaluation are also available in the replication package~\cite{schwab2025DanielDangoReplicationPackageAutomatedPromptEngineeringforTraceabilityLinkRecovery} for this thesis.


\section{Tree-of-Thought Prompting Evaluation}
\label{sec:Evaluation:tot_prompting}

Before evaluating the proposed \APE approaches, \ToT prompting by \citewithauthor{hulbert2023UsingTreeofThought} is evaluated for the task of \TLR in the \LiSSAf.
\autoref{tab:tree_of_thought} shows the systematic evaluation results for varying datasets using \ToT prompting compared to the simple \KISS \autoref{prompt:yes_no}.
...
\begin{table}
    ...
    \input{tables/TreeOfThought}
    \renewcommand{\arraystretch}{1}
    \caption{\ToT promting to compare performance with the \KISS \autoref{prompt:yes_no}.}
    \label{tab:tree_of_thought}
\end{table}

We can see in table \autoref{tab:tree_of_thought}, that the \ToT prompts by \citewithauthor{hulbert2023UsingTreeofThought} fail to outperform the simple \KISS \autoref{prompt:yes_no} for all tested datasets on the \gptmini \LLM.
Precision is slightly higher for several datasets, but recall and \fone are lower across the board.
The \ToT prompts are not promising for \TLR in the \LiSSAf.
Instead, this thesis focuses on the \APE approaches described in \autoref{ch:Approach}.
...
\autoref{prompt:yes_no} is chosen here as the initial prompt.
It is a \KISS zero-shot binary classification prompt.
As the \LiSSAf has already employed this prompt, the \APE algorithm is initialized with it as well.
Secondly, an optimization prompt is needed.
Therefore, \autoref{prompt:initial_optimization} has been arbitrarily selected.
Unlike \autoref{prompt:yes_no}, this prompt is designed by me, while implementing the naive prompt optimization approach.
Sentence completion through GitHub Copilot based on the \ac{gpt-copilot} model has also been utilized.

With these two prompts, the naive prompt optimization can be evaluated.
The results are quite consistent across different tested \LLMs.
The optimized prompt by \OAI's \gpt model is presented as an example of these.
The full results of this evaluation are included in the replication package~\cite{schwab2025DanielDangoReplicationPackageAutomatedPromptEngineeringforTraceabilityLinkRecovery}.
...
As in \autoref{tab:tree_of_thought} the \Average and \WeightedAverage are also included.
...
\begin{table}
    ...
    \renewcommand{\arraystretch}{1}
    \caption{Naive prompt optimization approach prompting \autoref{prompt:initial_optimization} to the \LLM to optimize the classification prompt \autoref{prompt:yes_no}.}
    \label{tab:naive_optimization}
    Results of the naive prompt optimization approach from \autoref{approach:sec:naive_iterative} without feedback.
    The \KISS zero-shot prompt \autoref{prompt:yes_no} is optimized using \autoref{prompt:initial_optimization}.
    The results are presented for both a single ($\iter = 1$) optimization step (\Quote{simple}) and five ($\iter = 5$) optimization steps (\Quote{iterative}).
\end{table}

We can see in \autoref{tab:naive_optimization}, that the \gpt and \gptmini \LLMs managed to improve the \fone scores for most datasets with both the simple and iterative optimization approaches.
Precision and recall are mostly consistent.
The \llama \LLM failed to classify any \TLs with the five times optimized prompt, resulting in recall and \fone of $0.0$ across all datasets.
This is because the output format was altered during optimization, thus failing in the \LiSSAF.
The prompt is included in the appendix as \autoref{prompt:naive_optimization_failure}.
...
To examine the optimized prompts, \autoref{prompt:yes_no_simple} shows the result of a single ($\iterl = 1$) iterative optimization run with \gpt.
The first application of the optimization prompt yielded an inclusion of the source and target types into the text of the prompt.
This is possible as a set of training data was also provided for the following, more sophisticated, prompt optimization approaches.
...
This very prompt is thus used for all simple \gpt evaluations in \autoref{tab:naive_optimization} for the task of \RtR.
...
This particular prompt is generated by \gpt using five ($\iterl = 5$) iteration steps.
...
On the \WARC dataset, \fone performance degraded slightly over \autoref{prompt:yes_no_simple} as we have seen in \autoref{tab:naive_optimization}.
What is especially noteworthy is that for \llama, the classification fails with \autoref{prompt:naive_optimization_failure}
As this shortcoming is expected, it is addressed by \autoref{approach:sec:gradient_descent}


\section{Simple Feedback Optimization}
\label{sec:Evaluation:simple_feedback_optimization}

The naive \APE process with feedback, as described in \autoref{approach:sec:naive_iterative_feedback}, utilizes more degrees of freedom than the naive approach in \autoref{sec:Evaluation:naive_optimization}.
To describe the different configurations used in this evaluation section, the parameters in \autoref{eq:feedback_parameters} are used.
The prompt is optimized \iter times.
\feed examples of misclassified \TLs are provided.
...
In addition to the optimization prompt \autoref{prompt:optimization_prompt_simple} used in \autoref{sec:Evaluation:naive_optimization_feedback}, an extended version is used here.
\autoref{prompt:feedback_initial} shows the additional details included in this prompt, which is added to the optimization prompt.
This provides the \LLM with more context on where the prompt failed to classify correctly.
The misclassified \TLs are be provided as additional context.
The \LLM can then use this information to improve the prompt.
Up to \feedl misclassified \TLs are provided.
The optimization process only utilizes a subset of the dataset, as explained in \autoref{approach:sec:naive_iterative_feedback}.
This training subset includes \feed source artifacts with four target artifact candidates each, resulting in up to twelve possible \TLs.
...
In this evaluation section, \iterl was always fully utilized, as the required \fone threshold of $1.0$ for the training subset was never reached.
...
The full optimization results are included in the replication package~\cite{schwab2025DanielDangoReplicationPackageAutomatedPromptEngineeringforTraceabilityLinkRecovery} for this thesis.
...
\begin{landscape}
    \begin{table}
        ...
        \renewcommand{\arraystretch}{1}
        \caption{Naive iterative prompt optimization approach considering previous misclassified \TLs}
        \label{tab:feedback_optimizer}
    \end{table}
\end{landscape}

They are visualized in \autoref{tab:feedback_optimizer}
...
\Todo{Add prompt to appendix! -> Wo ist der Prompt hin :monakaW:, der fehlt einfach in den Ergebnissen}


\subsection{Optimized Prompt Analysis}
\label{subsec:Evaluation:simple_feedback:optimized-prompt-analysis}

As the model now possesses additional knowledge of the dataset through the misclassified \TLs, we can see that the optimized prompts for each dataset start to diverge.
Compared to the naive optimization approach in \autoref{sec:Evaluation:naive_optimization}, the optimized prompts are now more tailored to the specific dataset.
To ease comparison, the results of \gpt for the \WARC dataset are presented here again.
\autoref{prompt:feedback_optimized_3_1} shows the result of one optimization run with $\feedl=3$ and $\iterl=1$.
...
This behaviour can be observed with the full results in the replication package~\cite{schwab2025DanielDangoReplicationPackageAutomatedPromptEngineeringforTraceabilityLinkRecovery}.
The prompt is also adapted to the \RtR task as indirectly instructed through the optimization prompt.
...
A fitting to the \RtR task is also present with indicators pointing to requirements directly.
...
\\
\omitt \newline
1. Look for shared terminology or concepts, such as specific file names, structures, or functionalities.
Pay attention to terms like "single header file," "universal header," and specific file names like "warc.h."\newline
\omitt \newline
...
\label{sec:Evaluation:gradient_based_optimization}
...
\begin{prompt}{Opimized WARC prompt \iterl = 3}{warc_gradient_optimized}
    \label{prompt:optimized_classificiation_prompt}
    \\
...
Evaluating \TLs between documentation and code might hide many requirements or only implicitly mention them.

\section{Application of an Optimized Prompt to Unfamiliar Datasets}
\label{sec:Evaluation:cross_dataset_evaluation}
