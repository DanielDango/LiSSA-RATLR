...
\Todo{Übergang, WTF ist der Absatz mit der Gleichung eigentlich?}
...
Context about why the prompt did not meet the requirements, also refered to as feedback, is not yet provided with \autoref{prompt:optimization_prompt_simplempt}.
...
\Todo{include Feedback Optimizer}
...
As such different prompt optimization algorithms are implemented and evaluated.
...
\Todo{Bissel kurz noch die subsection?}
\Todo{Ctrl+F nach ".\cite" (Quelle muss vor den Punkt)}