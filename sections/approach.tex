...
The full best performing \autoref{prompt:tot} can be found in the appendix.
...
\begin{figure}
    ...
    Highlighted in yellow is data, with x being the input which is queried with one of the prompting techniques to the \LLM and output representing the returned result of this query.
    ...
    \Todo{Add Highlight around prompt processes to illustrate the encolosed model?}
\end{figure}

In \autoref{fig:ToT_visualization} we can see the three different prompting techniques highlighted in green.
Among them are aside from the \ToT prompt also a zero-shot and \CoT prompt, to visualize the differences.
...
Thus, if any thought deviates, the \LLM might be led to a false conclusion.
\Todo{Source? I made it up, is not a valid source...}
...
The \promptoptimizer component will use the preprocessed data to optimize a given prompt.
The optimized prompt can then be used in the regular \LLM-based prompting step to classify \TLs.

The different prompt optimization algorithms to be used will be introduced in the following subsections.
...
\subsection{Naive Iterative \APE}
...
This naive \promptoptimizer implementation can be used for more sophisticated prompt optimization algorithms.
Furthermore, we are able to compare whether optimization results differ between naive and sophisticated approaches.

\subsection{\APE with \ProTeGi}
\label{approach:sec:gradient_descent}
The iterative optimization implementation created for \autoref{approach:sec:naive_iterative} is used as a foundation for the implementation.
...
The primary idea of this thesis is to explore \APE.
The naive approach of \autoref{approach:sec:naive_iterative} may yield good results already.
Feeding more information and processing logic into the optimization system is expected to present better results.
Especially suitable results may be reached faster by doing more processing on the local machine instead of expensive \API calls to the \LLM.
