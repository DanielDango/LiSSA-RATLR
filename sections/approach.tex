...
Next, naive iterative prompt optimization approaches with and without feedback are elaborated in \autoref{approach:sec:naive_iterative}.
...
This can be achieved by chaining multiple requests to an \LLM~\cite{long2023LargeLanguage}.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{graphics/tree_of_thought.drawio}
...
Examining the three different prompting techniques, we note a varying number of thoughts before the output is reached.
Thoughts are represented by white and orange rounded boxes, with orange ones being discarded during the thought process.
As for the zero-shot prompt, the \LLM reaches its conclusion without a thought process.
...
Prompting with \ToT addresses this, as we can see multiple thought explorers per level independently.
